<?php
  ...
  $stmt = $mysqli->prepare("SELECT COUNT(*), id FROM categories WHERE following_id=?");
  $stmt->bind_param("i", intval($_POST['id']));
  $stmt->execute();
  ...
  $stmt = $mysqli->prepare("SELECT COUNT(*), id FROM categories WHERE preceding_id=?");
  $stmt->bind_param("i", intval($_POST['id']));
  $stmt->execute();
  ...
  if (intval($preceding_ct_str) != 0 && intval($following_ct_str) != 0) {
    ...
    $stmt = $mysqli->prepare("UPDATE categories SET preceding_id=? WHERE id=?");
